...
export class EventResolver {
  ...
  async trendingEvents(): Promise<Event[] | undefined> {
    try {
      const eventPrice: EventPrice[] | undefined = await EventPrice.find({
        relations: ['event'],
        order: { sold: 'DESC' }
      });
  ...
  async createEvent(
    ...
    try {
      ...
      const organization: Organization | undefined = await Organization.findOne(
  ...
  async deleteEventById(
    ...
    try {
      ...
      const event: DeleteResult | undefined = await Event.delete({
        id: eventId,
        organization: { id: orgTeamMember.organization.id }
      });
      if (!event.affected) {
  ...
  async organization(@Root() parent: Event): Promise<Organization | undefined> {
    try {
      return await Event.findOne({
        where: { id: parent.id },
        relations: ['organization']
      }).then((event) => event?.organization);
    } catch (err: any) {
      console.log(err);
    }
  }
  ...
  async photos(@Root() parent: Event): Promise<EventPhoto[] | undefined> {
    try {
      return await EventPhoto.find({
  ...
  async timings(@Root() parent: Event): Promise<EventTiming[] | undefined> {
    try {
      return await EventTiming.find({
        where: { event: { id: parent.id } },
        relations: ['event']
      });
    } catch (err: any) {
  ...
  async prices(@Root() parent: Event): Promise<EventPrice[] | undefined> {
    try {
      return await EventPrice.find({
        where: { event: { id: parent.id } },
        relations: ['event']
      });
    } catch (err: any) {
  ...
  async reviews(@Root() parent: Event): Promise<Review[] | undefined> {
    try {
  ...
  async bookings(@Root() parent: Event): Promise<Booking[] | undefined> {
    try {
      return await Booking.find({
        where: { event: { id: parent.id } },
        relations: ['user', 'event', 'bookingItem'],
        order: { createdAt: 'DESC' }
      });
    } catch (err: any) {
      console.log(err);
