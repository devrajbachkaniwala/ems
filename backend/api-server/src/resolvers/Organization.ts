...
export class OrganizationResolver {
  ...
  async organization(
    ...
    try {
      const orgTeamMember: OrganizationTeamMember | undefined =
        await OrganizationTeamMember.findOne({
          where: { user: { id: req.userId } },
          relations: ['user', 'organization']
        });
      if (!orgTeamMember) {
        throw new ApolloError(
          'Requested user does not belong to the organization'
        );
      }
  ...
  // Query organization data by orgId
  @Query((type) => Organization)
  @Authorized('ORGANIZATION')
  async getOrganizationById(
    ...
    try {
      const orgTeamMember: OrganizationTeamMember | undefined =
        await OrganizationTeamMember.findOne({
          where: { user: { id: req.userId }, organization: { id: orgId } },
          relations: ['user', 'organization']
        });
      if (!orgTeamMember) {
        throw new ApolloError(
  ...
  async addTeamMember(
    ...
    try {
      const organizationTeamMember: OrganizationTeamMember | undefined =
        await OrganizationTeamMember.findOne({
          where: { user: { id: req.userId } },
          relations: ['user', 'organization']
        });
      ...
      const member: OrganizationTeamMember | undefined =
        await OrganizationTeamMember.findOne({
          where: { user: { email } },
          relations: ['user', 'organization']
        });
      ...
      if (member) {
        throw new ApolloError('User is a part of another organization');
      }

      const user: User | undefined = await User.findOne({ where: { email } });
      if (!user) {
        throw new ApolloError('User is not a registered user');
      }

      const orgTeamMember: OrganizationTeamMember =
        await OrganizationTeamMember.create({
          user,
          organization: organizationTeamMember.organization
        }).save();
  ...
  async removeTeamMember(
    ...
    try {
      ...
      if (!member) {
        throw new ApolloError(
          'User not found or User cannot remove themselves from team member'
        );
      }

      const organizationTeamMember: DeleteResult | undefined =
        await OrganizationTeamMember.delete({ id: member.id });

      if (!organizationTeamMember.affected) {
        throw new ApolloError(
          'User not found in team members of an organization'
        );
      }
  ...
  async createOrganization(
    ...
    try {
      ...
      const orgTeamMember: OrganizationTeamMember =
        await OrganizationTeamMember.create({ user, organization }).save();
  ...
  async deleteOrganization(
    ...
    try {
      ...
      const org: DeleteResult = await Organization.delete({
        id: orgTeamMember?.organization.id
      });
  ...
  async updateOrganization(
    @Arg('data', (type) => UpdateOrganizationInput)
    ...
    try {
      ...
      const org: Organization | undefined = await Organization.findOne({
        where: { id: orgTeamMember.organization.id }
      });
  ...
  async teamMembers(
    ...
    try {
      return await OrganizationTeamMember.find({
        where: { organization: { id: parent.id } },
        relations: ['user', 'organization']
      });
    } catch (err: any) {
      console.log(err);
  ...
  async events(@Root() parent: Organization): Promise<Event[] | undefined> {
    try {
  ...
  async bookingItems(
    ...
    try {
      return await BookingItem.find({
        where: { organization: { id: parent.id } },
        relations: ['booking', 'price', 'timing', 'organization']
      });
    } catch (err: any) {
      console.log(err);
    }
