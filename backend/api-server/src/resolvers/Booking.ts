...
export class BookingResolver {
  ...
  @Authorized('USER')
  async myBookings(@Ctx() { req }: IContext): Promise<Booking[] | undefined> {
    try {
  ...
  async addBooking(
    ...
    try {
      ...
      const event: Event | undefined = await Event.findOne({
        where: { id: data.eventId, organization: { id: organization.id } },
      ...
      const eventPrice: EventPrice | undefined = await EventPrice.findOne({
        where: { id: data.priceId, event: { id: event.id } },
        relations: ['event']
      });
      ...
      const eventTiming: EventTiming | undefined = await EventTiming.findOne({
        where: { id: data.timingId, event: { id: event.id } },
        relations: ['event']
      });
      ...
      const bookingItem: BookingItem = await BookingItem.create({
        qty: data.qty,
        booking,
        price: eventPrice,
        timing: eventTiming,
  ...
  async cancelBooking(
    ...
    try {
      const bookingItem: BookingItem | undefined = await BookingItem.findOne({
        where: {
          booking: { id: bookingId, user: { id: req.userId } },
          status: Not('cancel')
        },
  ...
  async bookingItem(@Root() parent: Booking): Promise<BookingItem | undefined> {
    try {
      return await BookingItem.findOne({
        where: { booking: { id: parent.id } },
        relations: ['booking', 'price', 'organization', 'timing']
      });
    } catch (err: any) {
      console.log(err);
    }
