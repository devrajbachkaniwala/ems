...
export class Validation {
  ...
  static password(password: string): boolean | IValidateError {
    ...
    if (!passwordRegExp.test(password)) {
      return {
        errMsg: 'Password should contain uppercase, lowercase, digit, symbol'
      };
    }
  ...
  static fullName(fullName: string): boolean | IValidateError {
    const fullNameRegExp: RegExp = /^[a-zA-Z\s]*$/;
    ...
    if (!fullNameRegExp.test(fullName)) {
      return {
        errMsg: 'Full Name should not contain digit, symbol or underscore'
      };
    }
